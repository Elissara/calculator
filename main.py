...
class Calculator(QMainWindow):
    def __init__(self):
        ...
        QFontDatabase.addApplicationFont('fonts/Rubik_Black_Italic.ttf')
    ...
    def add_temp(self, math_sign: str):
        if not self.temp.text() or self.get_math_sign() == '=':
            self.temp.setText(self.remove_trailing_zeros(self.entry.text()) + f' {math_sign} ')
            self.adjust_temp_font_size()
            self.entry.setText('0')
            self.adjust_entry_font_size()
    ...
    def disable_buttons(self, disable: bool) -> None:
        ...
        color = 'color: gray;' if disable else 'color: white;'
        self.change_buttons_color(color)

    def change_buttons_color(self, css_color: str) -> None:
        self.ui.btn_result.setStyleSheet(css_color)
    ...
    def adjust_temp_font_size(self) -> None:
        ...
        while self.get_temp_text_width() > self.temp.width() - 10:
            font_size -= 1
            self.temp.setStyleSheet('font-size: ' + str(font_size) + 'pt; color: gray;')
